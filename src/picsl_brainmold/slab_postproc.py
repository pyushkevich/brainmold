...
def make_paper_template(args):
    ...
    for i_slab, fimg in slab_indices.items():
        ...
        # Read the MRI slab
        mri_slab = sitk.ReadImage(tmp_slab)
...
class PaperTemplateLauncher:
    ...
    def __init__(self, parser):
        ...
        parser.add_argument("-i", dest="id", type=str, required=True, help="Specimen ID")
        parser.add_argument("-d", dest="dir", type=str, required=True, help="Brainmold output directory")
        parser.set_defaults(func = lambda args : self.run(args))
        
    def run(self, args):
        make_paper_template(args)
